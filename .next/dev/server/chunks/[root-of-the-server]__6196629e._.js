...
"[project]/src/app/api/tenant/neraca-saldo/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function calculateAccountBalanceWithDate(kodeAkun, tenantId, tipeAkun, saldoAwal, startDate, endDate) {
    const jurnalWhere = {
        tenantId,
        isApproved: true
    };
...
async function GET(request) {
    try {
        ...
        const accounts = await Promise.all(akunList.map(async (akun)=>{
            const saldo = await calculateAccountBalanceWithDate(akun.kodeAkun, tenantId, akun.tipeAkun, akun.saldoAwal, startDate, endDate);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__6196629e._.js.map